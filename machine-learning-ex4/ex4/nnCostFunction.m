...
J = (1 / m) * sum(sum(-y_mat .* log(y_pred) - (1 - y_mat) .* log(1 - y_pred)));
J = J + (lambda / (2 * m)) * (sum(sum(Theta1(:, 2:end) .^ 2)) + sum(sum(Theta2(:, 2:end) .^ 2)));

% Calculate gradient

for i = 1:m
    Delta3 = (y_pred(i, :) - y_mat(i, :))';
    Delta2 = Theta2' * Delta3 .* (hidden(i, :) .* (1 - hidden(i, :)))';
    Theta2_grad = Theta2_grad + Delta3 * hidden(i, :);
